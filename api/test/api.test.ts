...
ORDER BY name
  ...
  describe('toArrowIPC', () => {
    // Helper to verify Arrow IPC stream header
    ...
    test('basic query', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run('SELECT 1 as a, 2 as b, 3 as c');
        const ipc = result.toArrowIPC();
    ...
    test('integers', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          'SELECT i::INTEGER as val FROM range(100) t(i)'
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
        // Should contain data for 100 integers
        assert.isAbove(ipc.length, 100);
      });
    });

    test('various primitive types', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(`
          SELECT
    ...
    test('large result with multiple chunks', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          'SELECT i::BIGINT as val FROM range(50000) t(i)'
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
        // Should be large enough for 50k bigints (8 bytes each) plus overhead
        assert.isAbove(ipc.length, 50000);
      });
    });

    test('empty result', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run('SELECT 1 as a WHERE false');
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
    ...
    test('list type', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT [1, 2, 3]::INTEGER[] as arr, ['a', 'b']::VARCHAR[] as str_arr"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('struct type', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT {'name': 'Alice', 'age': 30}::STRUCT(name VARCHAR, age INTEGER) as person"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('nested types', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(`
          SELECT
            [[1, 2], [3, 4]]::INTEGER[][] as nested_list,
            {'items': [1, 2, 3], 'name': 'test'}::STRUCT(items INTEGER[], name VARCHAR) as struct_with_list
        `);
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('large strings', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT repeat('x', 10000) as long_str"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
        // Should contain the 10k character string
        assert.isAbove(ipc.length, 10000);
      });
    });

    test('blob data', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT '\\xDEADBEEF'::BLOB as binary_data"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('decimal', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT 123.456::DECIMAL(10, 3) as price"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('interval', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          "SELECT INTERVAL '1 year 2 months 3 days' as duration"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
    ...
    test('consistent output for same query', async () => {
      await withConnection(async (connection) => {
        const result1 = await connection.run('SELECT 42 as val');
        const result2 = await connection.run('SELECT 42 as val');

        const ipc1 = result1.toArrowIPC();
        const ipc2 = result2.toArrowIPC();
    ...
    test('enum type', async () => {
      await withConnection(async (connection) => {
        await connection.run("CREATE TYPE mood AS ENUM ('happy', 'sad', 'neutral')");
        const result = await connection.run(
          "SELECT 'happy'::mood as current_mood"
        );
        const ipc = result.toArrowIPC();
        assertValidIpcHeader(ipc);
      });
  ...
  describe('streamArrowIPC', () => {
    test('streams schema and chunks', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          'SELECT i::INTEGER as val FROM range(100) t(i)'
        );
        ...
        for await (const chunk of result.streamArrowIPC()) {
          chunks.push(chunk);
        }

        // Should have at least schema + 1 data chunk
        assert.isAtLeast(chunks.length, 2);

        // First chunk is schema (starts with IPC continuation marker)
        assert.equal(chunks[0][0], 0xff);
        assert.equal(chunks[0][1], 0xff);
        assert.equal(chunks[0][2], 0xff);
        assert.equal(chunks[0][3], 0xff);

        // Data chunks also start with continuation marker
        assert.equal(chunks[1][0], 0xff);
        assert.equal(chunks[1][1], 0xff);
        assert.equal(chunks[1][2], 0xff);
        assert.equal(chunks[1][3], 0xff);
      });
    });

    test('large result produces multiple chunks', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run(
          'SELECT i::BIGINT as val FROM range(10000) t(i)'
        );
        ...
        // Schema + data chunks (10k rows might span multiple chunks)
        assert.isAtLeast(chunks.length, 2);
      });
    });

    test('concatenated stream equals full IPC', async () => {
      await withConnection(async (connection) => {
        const result1 = await connection.run(
          'SELECT i::INTEGER as val FROM range(50) t(i)'
        );
        const fullIpc = result1.toArrowIPC();
        ...
        const chunks: Uint8Array[] = [];
        for await (const chunk of result2.streamArrowIPC()) {
          chunks.push(chunk);
        }
        ...
        // Both should be valid IPC (start with continuation marker)
        assert.equal(fullIpc[0], 0xff);
        assert.equal(streamedIpc[0], 0xff);
      });
    });

    test('empty result streams schema only', async () => {
      await withConnection(async (connection) => {
        const result = await connection.run('SELECT 1 as a WHERE false');
        ...
        // Should have just the schema (no data chunks since result is empty)
        assert.equal(chunks.length, 1);

        // Schema has valid IPC header
        assert.equal(chunks[0][0], 0xff);
        assert.equal(chunks[0][1], 0xff);
        assert.equal(chunks[0][2], 0xff);
        assert.equal(chunks[0][3], 0xff);
